...
use crate::app::{
    card, color_row, dropdown, labeled_slider, save_bar, section_header, toggle_row, KEYCAP_STYLES,
    TEXT_CAPS, TEXT_VARIANTS,
};
use crate::theme::Theme;
use eframe::egui::{ComboBox, Context, RichText, Ui};
use input_core::presets::ThemePreset;

pub fn render_keycap_tab(
    ui: &mut Ui,
    theme: &Theme,
    ctx: &Context,
    ...
    card(ui, theme, |ui| {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("Preset").color(theme.text_dim).size(13.0));
            ui.with_layout(
                eframe::egui::Layout::right_to_left(eframe::egui::Align::Center),
                |ui| {
                    let apply_btn = ui.add(
                        eframe::egui::Button::new(RichText::new("Apply").size(12.0))
                            .fill(theme.accent)
    ...
    card(ui, theme, |ui| {
        ...
        color_row(
            ...
            theme,
            "Primary",
            &mut app.config.keycap_primary.clone().unwrap_or_default(),
        );
        color_row(
            ui,
            theme,
            "Secondary",
            &mut app.config.keycap_secondary.clone().unwrap_or_default(),
        );
        ...
        if highlight_mods {
            color_row(
                ...
                theme,
                "Modifier Primary",
                &mut app.config.modifier_primary.clone().unwrap_or_default(),
            );
            color_row(
                ui,
                theme,
                "Modifier Secondary",
                &mut app.config.modifier_secondary.clone().unwrap_or_default(),
            );
        }
    });
    ...
    card(ui, theme, |ui| {
        ...
        app.config.text_size = if text_size <= 0.0 {
            None
        } else {
            Some(text_size)
        };
        ...
        color_row(
            ...
            theme,
            "Color",
            &mut app.config.text_color.clone().unwrap_or_default(),
        );
        ...
        if highlight_mods {
            color_row(
                ...
                theme,
                "Modifier Color",
                &mut app.config.text_modifier_color.clone().unwrap_or_default(),
            );
        }
    });
    ...
    card(ui, theme, |ui| {
        ...
        if border_enabled {
            color_row(
                ...
                theme,
                "Color",
                &mut app.config.border_color.clone().unwrap_or_default(),
            );
            ...
            if highlight_mods {
                color_row(
                    ...
                    theme,
                    "Modifier Color",
                    &mut app.config.border_modifier_color.clone().unwrap_or_default(),
                );
            }
        }
    });
    ...
    card(ui, theme, |ui| {
        ...
        if bg_enabled {
            color_row(
                ...
                theme,
                "Color",
                &mut app.config.background_color.clone().unwrap_or_default(),
            );
        }
    });
