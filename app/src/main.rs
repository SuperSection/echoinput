...
fn create_platform_components(
    ...
    let renderer: Box<dyn OverlayRenderer> = if is_wayland {
        ...
        Box::new(overlay_x11::X11Renderer::with_shutdown(bus, shutdown.clone()))
    };
...
fn create_platform_components(
    ...
    use overlay_windows::WindowsRenderer;
    use platform_windows::WindowsCapture;
...
fn create_platform_components(
    ...
    use overlay_macos::MacRenderer;
    use platform_macos::MacosCapture;
            ...
            match self.active_tab {
                ...
                SettingsTab::About => {
                    ...
                    Self::card(ui, &theme, |ui| {
                        ...
                        ui.label(eframe::egui::RichText::new("Repository: github.com/echoinput/echoinput").color(theme.text_muted).size(12.0));
                    });
                }
            }
        });
